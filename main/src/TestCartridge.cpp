...
namespace ConsoleGameEngine
{
	...
	void TestCartridge::Update(double deltaTimeMs)
	{
		...
		nestedCanvas->Clear(BackgroundColour::Black);
	...
	void TestCartridge::Render(Canvas& canvas)
	{
		...
		// Draw the nested canvas.
		Utils::DrawTextCentered(canvas,nestedOffset.Y-2,"Nested Canvas", ForegroundColour::White);
		canvas.DrawCanvas(*nestedCanvas, nestedOffset.X, nestedOffset.Y);
	}
